...
-- Chuyển sang container XEPDB1 và schema CQ09 để tạo bảng/dữ liệu chuẩn xác
...
BEGIN
    FOR v IN (
        ...
        WHERE owner = SYS_CONTEXT('USERENV', 'CURRENT_SCHEMA')
          AND view_name IN ('V_NGUOIDUNG_HE_THONG','V_MY_ACCOUNT')
    ) LOOP
...
BEGIN
    FOR t IN (
        ...
        WHERE owner = SYS_CONTEXT('USERENV', 'CURRENT_SCHEMA')
          AND table_name IN ('DONTHUOC','HSBA_DV','HSBA','BENHNHAN','NHANVIEN','THONGBAO')
...
PROMPT ===== 3B. TAO VIEW NGHIEP VU CHO PHAN HE 2 =====

CREATE OR REPLACE VIEW vw_nhanvien_canhan AS
SELECT MANV, HOTEN, PHAI, NGAYSINH, CMND, QUEQUAN, SODT, VAITRO, CHUYENKHOA
FROM NHANVIEN
...
SELECT MANV, HOTEN
FROM NHANVIEN
WHERE VAITRO = N'Ká»¹ thuáº­t viÃªn';
...
SELECT h.MAHSBA, h.MABN, b.TENBN, b.PHAI, b.NGAYSINH, h.NGAY,
...
CREATE OR REPLACE TRIGGER trg_vw_bacsi_hsba_u
...
BEGIN
    ...
    WHERE MAHSBA = :OLD.MAHSBA
      AND MABS = SYS_CONTEXT('USERENV', 'SESSION_USER');
END;
/
...
CREATE OR REPLACE TRIGGER trg_vw_bacsi_donthuoc_i
INSTEAD OF INSERT ON vw_bacsi_donthuoc
FOR EACH ROW
DECLARE
    v_count NUMBER;
BEGIN
    ...
    IF v_count = 0 THEN
        RAISE_APPLICATION_ERROR(-20031, 'Bac si chi duoc them don thuoc cho HSBA do minh phu trach.');
    END IF;

    INSERT INTO DONTHUOC(MAHSBA, NGAYDT, TENTHUOC, LIEUDUNG)
    VALUES (:NEW.MAHSBA, :NEW.NGAYDT, :NEW.TENTHUOC, :NEW.LIEUDUNG);
END;
/

CREATE OR REPLACE TRIGGER trg_vw_bacsi_donthuoc_u
INSTEAD OF UPDATE ON vw_bacsi_donthuoc
FOR EACH ROW
BEGIN
    UPDATE DONTHUOC
    SET LIEUDUNG = :NEW.LIEUDUNG
    WHERE MAHSBA = :OLD.MAHSBA
      AND NGAYDT = :OLD.NGAYDT
      AND TENTHUOC = :OLD.TENTHUOC
      AND EXISTS (
          SELECT 1 FROM HSBA h
          WHERE h.MAHSBA = :OLD.MAHSBA
            AND h.MABS = SYS_CONTEXT('USERENV', 'SESSION_USER')
      );
END;
/

CREATE OR REPLACE TRIGGER trg_vw_bacsi_donthuoc_d
INSTEAD OF DELETE ON vw_bacsi_donthuoc
FOR EACH ROW
BEGIN
...
SELECT dv.MAHSBA, dv.LOAIDV, dv.NGAYDV, dv.MAKTV, nv.HOTEN AS HOTEN_KTV, dv.KETQUA
FROM HSBA_DV dv
JOIN HSBA h ON h.MAHSBA = dv.MAHSBA
LEFT JOIN NHANVIEN nv ON nv.MANV = dv.MAKTV
WHERE h.MABS = SYS_CONTEXT('USERENV', 'SESSION_USER');

CREATE OR REPLACE TRIGGER trg_vw_bacsi_hsbadv_i
...
BEGIN
    ...
    IF v_count = 0 THEN
        RAISE_APPLICATION_ERROR(-20032, 'Bac si chi duoc chi dinh dich vu cho HSBA do minh phu trach.');
    END IF;

    INSERT INTO HSBA_DV(MAHSBA, LOAIDV, NGAYDV, MAKTV, KETQUA)
    VALUES (:NEW.MAHSBA, :NEW.LOAIDV, :NEW.NGAYDV, :NEW.MAKTV, :NEW.KETQUA);
END;
/

CREATE OR REPLACE TRIGGER trg_vw_bacsi_hsbadv_u
...
CREATE OR REPLACE TRIGGER trg_vw_bacsi_hsbadv_d
